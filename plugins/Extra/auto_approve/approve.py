...
async def auto_approve(client, message: ChatJoinRequest):
    if AUTO_APPROVE_MODE == True:
        ...
        text = f"<b>Salut {message.from_user.mention} 👋,\n\nTa Requete D'approbation {message.chat.title} a été approuvé.\n\nPropulsé Par - {CHNL_LNK}</b>"
    ...
    if data.split("-", 1)[0] == "VJ":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            await message.reply(f"<b>Vous avez rejoint via le lien de parrainage de l'utilisateur ID {user_id}\n\nEnvoyez /start à nouveau pour utiliser le bot</b>")
            num_referrals = await get_referal_users_count(user_id)
            await client.send_message(chat_id = user_id, text = "<b>{} a démarré le bot avec votre lien de parrainage\n\nTotal filleuls - {}</b>".format(message.from_user.mention, num_referrals))
            if num_referrals == REFERAL_COUNT:
                ...
                if seconds > 0:
                    ...
                    await db.update_user(user_data)  # Utiliser la méthode update_user pour mettre à jour les données
                    await delete_all_referal_users(user_id)
                    await client.send_message(chat_id = user_id, text = "<b>Vous avez complété votre parrainage avec succès.\n\nVous avez été ajouté au Premium pour {}</b>".format(REFERAL_PREMEIUM_TIME))
                    return 
        else:
            if PREMIUM_AND_REFERAL_MODE == True:
                buttons = [[
                    ...
                    InlineKeyboardButton('Rejoindre les mises à jour', url=CHNL_LNK)
                ]]
            else:
                buttons = [[
                    InlineKeyboardButton('⤬ Anime Terr ⤬', url=f'http://t.me/Anime_Trrr')
                ],[
                    InlineKeyboardButton('Offre VIP', callback_data="shortlink_info"),
                    InlineKeyboardButton('Groupe de films', url=GRP_LNK)
                ],[
            ...
            if CLONE_MODE == True:
                buttons.append([InlineKeyboardButton('Créer son propre bot clone', callback_data='clone')])
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton("• Télecharger •", url=download),
                        InlineKeyboardButton('• Regarder •', url=stream)
                    ],[
                        InlineKeyboardButton("• Regarder en Ligne •", web_app=WebAppInfo(url=stream))
                    ]]
    ...
    if data.startswith("sendfiles"):
        ...
        text = "<b>✅ Votre fichier est prêt, cliquez sur Télécharger maintenant puis ouvrez le lien pour obtenir le fichier\n\n</b>"
        if PREMIUM_AND_REFERAL_MODE == True:
        ...
        if temp.SHORT.get(user)==None:
            await message.reply_text(text="<b>Veuillez rechercher à nouveau dans le groupe</b>")
        else:
            chat_id = temp.SHORT.get(user)
        settings = await get_settings(chat_id)
        pre = 'filep' if settings['file_secure'] else 'file'
        if settings['is_shortlink'] and not await db.has_premium_access(user):
            ...
            text = "<b>✅ Votre fichier est prêt, cliquez sur Télécharger maintenant puis ouvrez le lien pour obtenir le fichier\n\n</b>"
            if PREMIUM_AND_REFERAL_MODE == True:
                text += "<b>Si vous souhaitez des fichiers directs sans ouvrir de lien ni voir de publicités, souscrivez à un abonnement premium ☺️\n\n💶 Envoyez /plan pour voir les offres d'abonnement</b>"
            k = await client.send_message(chat_id=message.from_user.id, text=text, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(1200)
            await k.edit("<b>✅ Votre message a été supprimé avec succès</b>")
            return
