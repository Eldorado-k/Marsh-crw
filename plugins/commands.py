...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            InlineKeyboardButton('⤬ ᴀɴɪᴍᴇ ᴛᴇʀʀ ⤬', url=f'http://t.me/Anime_Terr')
        ],[
            InlineKeyboardButton('Groupe Support', url=f'https://t.me/{SUPPORT_CHAT}'),
            InlineKeyboardButton('ZFlix-Team', url=GRP_LNK)
        ],[
            InlineKeyboardButton('BotZFlix', url=CHNL_LNK)
        ]]
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('Offre VIP', callback_data="shortlink_info"),
                InlineKeyboardButton('ZFlix-Team', url=GRP_LNK)
            ],[
                InlineKeyboardButton('Aide', callback_data='help'),
                InlineKeyboardButton('À Propos', callback_data='about')
            ],[
                InlineKeyboardButton('Premium & Parrainage', callback_data='subscription')
            ],[
                InlineKeyboardButton('BotZFlix', url=CHNL_LNK)
            ]]
        else:
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        try:btn = [[InlineKeyboardButton("Chaîne de sauvegarde", url=invite_link.invite_link)]]
            ...
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
                    text = "**🕵️ Vous n'avez pas rejoint ma chaîne de sauvegarde. Veuillez d'abord rejoindre la chaîne puis réessayer**"
                else:
                    await db.set_msg_command(message.from_user.id, com=message.command[1])
                    text = "**🕵️ Vous n'avez pas rejoint ma chaîne de sauvegarde. Veuillez d'abord rejoindre la chaîne**"
            else:
                text = "**🕵️ Vous n'avez pas rejoint ma chaîne de sauvegarde. Veuillez d'abord rejoindre la chaîne puis réessayer**"
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('Rejoindre la chaîne des mises à jour', url=CHNL_LNK)
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('⤬ Ajoutez-moi à votre groupe ⤬', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ...
        if CLONE_MODE == True:
            buttons.append([InlineKeyboardButton('Créer votre propre bot clone', callback_data='clone')])
        reply_markup = InlineKeyboardMarkup(buttons)      
        await message.reply_photo(
            photo=random.choice(PICS),
            caption=script.START_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME),
    ...
    if data.split("-", 1)[0] == "VJ":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            await message.reply(f"<b>Vous avez rejoint en utilisant le lien de parrainage de l'utilisateur avec l'ID {user_id}\n\nEnvoyez /start à nouveau pour utiliser le bot</b>")
            num_referrals = await get_referal_users_count(user_id)
            await client.send_message(chat_id = user_id, text = "<b>{} a démarré le bot avec votre lien de parrainage\n\nTotal des filleuls - {}</b>".format(message.from_user.mention, num_referrals))
            if num_referrals == REFERAL_COUNT:
                ...
                if seconds > 0:
                    ...
                    await db.update_user(user_data)  # Utilise la méthode update_user pour mettre à jour ou insérer des données utilisateur
                    await delete_all_referal_users(user_id)
                    await client.send_message(chat_id = user_id, text = "<b>Vous avez complété avec succès le total de parrainages.\n\nVous avez été ajouté en Premium pour {}</b>".format(REFERAL_PREMEIUM_TIME))
                    return 
        else:
            if PREMIUM_AND_REFERAL_MODE == True:
                buttons = [[
                    InlineKeyboardButton('⤬ Ajoutez-moi à votre groupe ⤬', url=f'http://t.me/Animr_Terr')
                ],[
                    InlineKeyboardButton('Offre VIP', callback_data="shortlink_info"),
                    InlineKeyboardButton('Groupe de films', url=GRP_LNK)
                ],[
                ...
                buttons = [[
                    InlineKeyboardButton('⤬ Anime Terr ⤬', url=f'http://t.me/Anime_Terr')
                ],[
                    InlineKeyboardButton('Offre VIP', callback_data="shortlink_info"),
                    InlineKeyboardButton('Groupe de films', url=GRP_LNK)
                ],[
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton("• Telecharger •", url=download),
                        InlineKeyboardButton('• Regarder •', url=stream)
                    ],[
                        InlineKeyboardButton("• Regarder Sur Le site •", web_app=WebAppInfo(url=stream))
                    ]]
            ...
            await asyncio.sleep(1) 
        await sts.delete()
        k = await client.send_message(chat_id = message.from_user.id, text=f"<blockquote><b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nCe message sera supprimé dans <b><u>10 minutes</u> 🫥 <i></b>(pour des raisons de droits d'auteur)</i>.\n\n<b><i>Veuillez transférer ce message à vos messages sauvegardés ou dans un chat privé.</i></b></blockquote>")
        await asyncio.sleep(600)
        for x in filesarr:
            await x.delete()
        await k.edit_text("<b>✅ Votre message a été supprimé avec succès</b>")  
        return
        ...
        if str(message.from_user.id) != str(userid):
            return await message.reply_text(text="<b>Lien invalide ou expiré</b>", protect_content=True)
        is_valid = await check_token(client, userid, token)
        if is_valid == True:
            text = "<b>Bonjour {} 👋,\n\nVous avez complété la vérification...\n\nVous avez maintenant un accès illimité jusqu'à aujourd'hui, profitez-en !\n\n</b>"
            if PREMIUM_AND_REFERAL_MODE == True:
                text += "<b>Si vous voulez des fichiers directs sans vérification, achetez un abonnement premium ☺️\n\n💶 Envoyez /plan pour voir les offres d'abonnement</b>"           
    ...
    if data.startswith("sendfiles"):
        ...
        btn = [[
            InlineKeyboardButton('Télécharger maintenant', url=g)
        ]]
        if settings['tutorial']:
            btn.append([InlineKeyboardButton('Comment télécharger', url=await get_tutorial(chat_id))])
        text = "<b>✅ Votre fichier est prêt, cliquez sur le bouton Télécharger maintenant puis ouvrez le lien pour obtenir le fichier\n\n</b>"
        if PREMIUM_AND_REFERAL_MODE == True:
            text += "<b>Si vous voulez des fichiers directs sans ouvrir de lien ni voir de publicités, achetez un abonnement premium ☺️\n\n💶 Envoyez /plan pour voir les offres d'abonnement</b>"
        k = await client.send_message(chat_id=message.from_user.id, text=text, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(300)
        await k.edit("<b>✅ Votre message a été supprimé avec succès</b>")
        return
        ...
        if not files:
            return await message.reply('<b><i>Aucun fichier correspondant</b></i>')
        filesarr = []
        for file in files:
            file_id = file["file_id"]
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    btn = [[
                        InlineKeyboardButton("Vérifier", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                    ],[
                        InlineKeyboardButton("Comment vérifier", url=VERIFY_TUTORIAL)
                    ]]
                    text = "<b>Bonjour {} 👋,\n\nVous n'êtes pas vérifié aujourd'hui, veuillez cliquer sur Vérifier pour obtenir un accès illimité pour aujourd'hui</b>"
                    if PREMIUM_AND_REFERAL_MODE == True:
                        text += "<b>Si vous voulez des fichiers directs sans vérification, achetez un abonnement premium ☺️\n\n💶 Envoyez /plan pour voir les offres d'abonnement</b>"
                    ...
                    return
            if STREAM_MODE == True:
                button = [[InlineKeyboardButton('Stream et Télécharger', callback_data=f'generate_stream_link:{file_id}')]]
                reply_markup=InlineKeyboardMarkup(button)
            else:
                reply_markup = None
            msg = await client.send_cached_media(
        ...
        if temp.SHORT.get(user)==None:
            await message.reply_text(text="<b>Veuillez effectuer une nouvelle recherche dans le groupe</b>")
        else:
            chat_id = temp.SHORT.get(user)
        settings = await get_settings(chat_id)
        pre = 'filep' if settings['file_secure'] else 'file'
    ...
    if not files_:
        ...
        try:
            ...
            btn = [[InlineKeyboardButton("✅ Obtenir le fichier à nouveau ✅", callback_data=f'del#{file_id}')]]
            k = await msg.reply(text=f"<blockquote><b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nCe message sera supprimé dans <b><u>10 minutes</u> 🫥 <i></b>(pour des raisons de droits d'auteur)</i>.\n\n<b><i>Veuillez transférer ce message à vos messages sauvegardés ou dans un chat privé.</i></b></blockquote>")
            await asyncio.sleep(600)
            await msg.delete()
            await k.edit_text("<b>✅ Votre message a été supprimé avec succès. Si vous voulez le fichier à nouveau, cliquez sur le bouton ci-dessous</b>",reply_markup=InlineKeyboardMarkup(btn))
            return
        except:
            pass
        return await message.reply('Aucun fichier correspondant.')
    files = files_
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        await message.reply_text(f"**Détails de votre abonnement :\n\nTemps restant : {remaining_time}\n\nDate d'expiration : {expiry_time}**")
    else:
        btn = [ 
            [InlineKeyboardButton("Obtenir un essai gratuit de 5 minutes ☺️", callback_data="get_trail")],
            [InlineKeyboardButton("Acheter un abonnement : Supprimer les pubs", callback_data="buy_premium")],
            [InlineKeyboardButton("⚠️ Fermer / Supprimer ⚠️", callback_data="close_data")]
        ]
        reply_markup = InlineKeyboardMarkup(btn)
        m=await message.reply_sticker("CAACAgIAAxkBAAIBTGVjQbHuhOiboQsDm35brLGyLQ28AAJ-GgACglXYSXgCrotQHjibHgQ")         
        await message.reply_text(f"**😢 Vous n'avez pas d'abonnement premium.\n\nDécouvrez nos offres premium avec /plan**",reply_markup=reply_markup)
