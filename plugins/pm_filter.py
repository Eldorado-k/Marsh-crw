...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('Chaine de mis à jour', url=CHNL_LNK)
        ]]
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('AntiFlix', url='t.me/AntiFlix_A'),
                InlineKeyboardButton('ZFlix-Team', url=GRP_LNK)
            ],[
                InlineKeyboardButton('Aide', callback_data='help'),
                InlineKeyboardButton('À Propos', callback_data='about')
            ],[
                InlineKeyboardButton('Premium & Parainage', callback_data='subscription')
            ],[
                InlineKeyboardButton('Chaîne de mis à jour', url=CHNL_LNK)
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('⤬ Anime Terr ⤬', url=f'http://t.me/Anime_Terr')
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        try:
            ...
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
                    text = "**🕵️ Vous n'avez pas rejoint mon canal de sauvegarde. Rejoignez d'abord le canal, puis réessayez**"
                else:
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('AntiFlix', url="AntiFlix_A"),
                InlineKeyboardButton('ZFlix-Team', url=GRP_LNK)
            ],[
                InlineKeyboardButton('Aide', callback_data='help'),
                InlineKeyboardButton('À Propos', callback_data='about')
                ...
                InlineKeyboardButton('Chaîne de mise à jour', url=CHNL_LNK)
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('⤬ Anime Terr ⤬', url=f'http://t.me/Anime_Terr')
    ...
    if data.split("-", 1)[0] == "VJ":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            await message.reply(f"<b>Vous avez rejoint en utilisant le lien de parrainage de l'utilisateur avec l'ID {user_id}\n\nnvoyez /start à nouveau pour utiliser le bot</b>")
            num_referrals = await get_referal_users_count(user_id)
            await client.send_message(chat_id = user_id, text = "<b>{} a Démarrée le bot avec ton lien de parrainage\n\nTotal Parainez - {}</b>".format(message.from_user.mention, num_referrals))
            if num_referrals == REFERAL_COUNT:
                ...
                if seconds > 0:
                    ...
                    await client.send_message(chat_id = user_id, text = "<b>Vous avez complété avec succès le total des parrainages.\n\nVous obtenez Premium pour {}</b>".format(REFERAL_PREMEIUM_TIME))
                    return 
        else:
            if PREMIUM_AND_REFERAL_MODE == True:
                buttons = [[
                    ...
                    InlineKeyboardButton('AntiFlix', url="AntiFlix"),
                    InlineKeyboardButton('ZFlix-Team', url=GRP_LNK)
                ],[
                    InlineKeyboardButton('Aide', callback_data='help'),
                    InlineKeyboardButton('À Propos', callback_data='about')
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton("• Regarder sur notre site •", web_app=WebAppInfo(url=stream))
                    ]]
            ...
            await asyncio.sleep(1) 
        await sts.delete()
        k = await client.send_message(chat_id = message.from_user.id, text=f"<blockquote><b><u>❗️❗️❗️ IMPORTANT ❗️❗️❗️</u></b>\n\nCe message sera supprimé dans<b><u>10 minutes</u>🫥</b><i>(pour des raisons de droits d'auteur)</i>\n\n<b><i>Merci de transférer ce message à vos messages sauvegardés ou dans un chat privé.</i></b></blockquote>")
        await asyncio.sleep(600)
        for x in filesarr:
            await x.delete()
        await k.edit_text("<b>✅ Le message a été éffacé</b>")  
        return
    
    elif data.split("-", 1)[0] == "DSTORE":
        sts = await message.reply("<b>Veuillez patientez...</b>")
    ...
    if data.startswith("sendfiles"):
        ...
        if settings['tutorial']:
            btn.append([InlineKeyboardButton('Comment Téléchargé', url=await get_tutorial(chat_id))])
        text = "<b>✅ Le Fichier est Prêt. clique sur Telecharger pour récupéré le fichier\n\n</b>"
        if PREMIUM_AND_REFERAL_MODE == True:
            text += "<b>ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴅɪʀᴇᴄᴛ ғɪʟᴇꜱ ᴡɪᴛʜᴏᴜᴛ ᴀɴʏ ᴏᴘᴇɴɪɴɢ ʟɪɴᴋ ᴀɴᴅ ᴡᴀᴛᴄʜɪɴɢ ᴀᴅs ᴛʜᴇɴ ʙᴜʏ ʙᴏᴛ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ☺️\n\n💶 ꜱᴇɴᴅ /plan ᴛᴏ ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ</b>"
        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    ...
                    return
            if STREAM_MODE == True:
                button = [[InlineKeyboardButton('Stream mode', callback_data=f'generate_stream_link:{file_id}')]]
                reply_markup=InlineKeyboardMarkup(button)
            else:
                reply_markup = None
            msg = await client.send_cached_media(
        ...
        if temp.SHORT.get(user)==None:
            await message.reply_text(text="<b>Veuillez Recherchez À Nouveau dans le groupe</b>")
        else:
            chat_id = temp.SHORT.get(user)
        settings = await get_settings(chat_id)
        pre = 'filep' if settings['file_secure'] else 'file'
        if settings['is_shortlink'] and not await db.has_premium_access(user):
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start={pre}_{file_id}")
            btn = [[
                InlineKeyboardButton('Téléchargé', url=g)
            ]]
            if settings['tutorial']:
                btn.append([InlineKeyboardButton('Comment Télécharger', url=await get_tutorial(chat_id))])
            text = "<b>✅ Ton fichier est prêt, clique sur le bouton « Télécharger maintenant » puis ouvre le lien pour obtenir le fichier.\n\n</b>"
            if PREMIUM_AND_REFERAL_MODE == True:
                text += "<b>Si tu veux des fichiers directs sans ouvrir de lien ni regarder de pubs, alors achète un abonnement bot. ☺️\n\n💶 Envoie /plan pour acheter un abonnement.</b>"
            k = await client.send_message(chat_id=message.from_user.id, text=text, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(1200)
            await k.edit("<b>✅ Le message a été supprimé avec succès.</b>")
            return
    user = message.from_user.id
        ...
        try:
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    ...
                    return
            if STREAM_MODE == True:
                button = [[InlineKeyboardButton('Diffuser en streaming et télécharger.', callback_data=f'generate_stream_link:{file_id}')]]
                reply_markup=InlineKeyboardMarkup(button)
            else:
                reply_markup = None
            msg = await client.send_cached_media(
            ...
            btn = [[InlineKeyboardButton("✅ Récupérer le fichier à nouveau. ✅", callback_data=f'del#{file_id}')]]
            k = await msg.reply(text=f"<blockquote><b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b><br><br>Ce message sera supprimé dans <b><u>10 minutes</u> 🫥</b> <i>(en raison de problèmes de droits d’auteur)</i>.<br><br><b><i>Merci de transférer ce message vers tes messages sauvegardés ou toute conversation privée.</i></b></blockquote>")
            await asyncio.sleep(600)
            await msg.delete()
            await k.edit_text("<b>✅ Ton message a été supprimé avec succès. Si tu veux le recevoir à nouveau, clique sur le bouton ci-dessous.</b>",reply_markup=InlineKeyboardMarkup(btn))
            return
        except:
            pass
