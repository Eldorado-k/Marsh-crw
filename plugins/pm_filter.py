    ...
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if settings['fsub'] != None:
            try:
                ...
                if btn:
                    btn.append([InlineKeyboardButton("Débloquer moi 🔕", callback_data=f"unmuteme#{int(user_id)}")])
                    await client.restrict_chat_member(chatid, message.from_user.id, ChatPermissions(can_send_messages=False))
                    await message.reply_photo(photo=random.choice(PICS), caption=f"👋 Salut {message.from_user.mention},\n\nVeuillez rejoindre la chaîne puis cliquer sur le bouton 'Débloquer moi'. 😇", reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML)
                    return
        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>Recherche de {message.text} 🔍</i></b>")
        ...
        if total_results == 0:
            ...
            return await message.reply_text(f"<b>Salut {message.from_user.mention}, {str(total_results)} résultats trouvés dans ma base de données pour votre recherche {search}. \n\nCeci est un groupe de support donc vous ne pouvez pas obtenir de fichiers ici...\n\nRejoignez et cherchez ici - {GRP_LNK}</b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>Recherche de {content} 🔍</i></b>", reply_to_message_id=message.id)
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>Recherche de {movie} 🔍</i></b>")
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="Sélectionnez votre année", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ Retour à l'accueil ↭", callback_data=f"fy#homepage#{key}")])
...
async def episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="Sélectionnez votre épisode", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ Retour à l'accueil ↭", callback_data=f"fe#homepage#{key}")])
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 Sélectionnez votre langue 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ Retour à l'accueil ↭", callback_data=f"fl#homepage#{key}")])
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="⇊ Sélectionnez votre qualité ⇊", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if not free_trial_status:            
            ...
            new_text = "**Vous pouvez utiliser l'essai gratuit pendant 5 minutes à partir de maintenant 😀\n\nआप अब से 5 मिनट के लिए निःशुल्क ट्रायल का उपयोग कर सकते हैं 😀**"        
            await query.message.edit_text(text=new_text)
            return
        else:
            new_text= "**🤣 Vous avez déjà utilisé l'essai gratuit. Veuillez souscrire à un abonnement ici 👉 /plans**"
            await query.message.edit_text(text=new_text)
            return
            
    elif query.data == "buy_premium":
        btn = [[            
            InlineKeyboardButton("✅ Envoyez votre reçu de paiement ici ✅", url = OWNER_LINK)
        ]
            for admin in ADMINS
        ]
        btn.append(
            [InlineKeyboardButton("⚠️Fermer / Supprimer⚠️", callback_data="close_data")]
        )
        reply_markup = InlineKeyboardMarkup(btn)
    ...
    if clicked == typed:
        ...
        await query.answer(f"Hé {query.from_user.first_name}, ce n'est pas votre demande de film. Demandez le vôtre !", show_alert=True)  # ← Traduit
    ...
    await query.answer('Débloque le bot, mon ami !', show_alert=True)  # ← Traduit
    ...
    if userid == 0:
        await query.answer("Vous êtes un admin anonyme !", show_alert=True)  # ← Traduit
        return
    try:
        btn = await pub_is_subscribed(client, query, settings['fsub'])
        if btn:
            await query.answer("Rejoignez la chaîne donnée avant de cliquer sur le bouton de réactivation", show_alert=True)  # ← Traduit
        else:
            await client.unban_chat_member(query.message.chat.id, user_id)
            await query.answer("Réactivé avec succès !", show_alert=True)  # ← Traduit
            try:
                await query.message.delete()
            except:
                return
    except:
        await query.answer("Ce n'est pas pour toi, mon cher", show_alert=True)  # ← Traduit

elif query.data.startswith("del"):
    ident, file_id = query.data.split("#")
    files_ = await get_file_details(file_id)
    if not files_:
        return await query.answer('Aucun fichier trouvé.', show_alert=True)  # ← Traduit
    files = files_
    title = files['file_name']
    size = get_size(files['file_size'])
    f_caption = files['caption']
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, query):
        await query.answer("Rejoignez notre chaîne de sauvegarde, mon ami ! 😒", show_alert=True)  # ← Traduit
        return
    ...
    files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
    await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
    await query.answer(f"Hé {query.from_user.first_name}, tous les fichiers de cette page ont été envoyés avec succès en MP !", show_alert=True)  # ← Traduit
    ...
    await query.message.edit_text("<b>La suppression des fichiers commencera dans 5 secondes !</b>")  # ← Traduit
        ...
        try:
            for file in files:
                ...
                if result.deleted_count:
                    logger.info(f'Fichier trouvé pour votre requête {keyword} ! {file_name} supprimé avec succès.')
                deleted += 1
                if deleted % 50 == 0:
                    await query.message.edit_text(f"<b>Suppression en cours... {deleted} fichiers supprimés pour la requête {keyword}.\n\nVeuillez patienter...</b>")  # ← Traduit
        except Exception as e:
            logger.exception(e)
            await query.message.edit_text(f'Erreur : {e}')
        else:
            await query.message.edit_text(f"<b>Opération terminée !\n\n{deleted} fichiers supprimés pour la requête {keyword}.</b>")  # ← Traduit
    ...
    if (
        ...
        await query.answer("Vous n'avez pas les droits pour faire ça !", show_alert=True)  # ← Traduit
        return
    title = query.message.chat.title
    ...
    if settings is not None:
        buttons = [
            ...
            [
                InlineKeyboardButton('Protéger le contenu',
            ...
            [
                InlineKeyboardButton('Vérification orthographique',
                                     callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
                InlineKeyboardButton('✔ Activé' if settings["spell_check"] else '✘ Désactivé',
                                     callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
            ],
            [
                InlineKeyboardButton('Message de bienvenue', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}'),
                InlineKeyboardButton('✔ Activé' if settings["welcome"] else '✘ Désactivé',
                                     callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}')
            ],
            [
                InlineKeyboardButton('Suppression automatique',
                                     callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{str(grp_id)}'),
                InlineKeyboardButton('5 min' if settings["auto_delete"] else '✘ Désactivé',
                                     callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{str(grp_id)}')
            ],
            [
                InlineKeyboardButton('Filtre automatique',
            ...
            [
                InlineKeyboardButton('Lien court',
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=f"<b>Modifiez les paramètres de {title} comme vous le souhaitez ⚙</b>",  # ← Traduit
    ...
    settings = await get_settings(grp_id)
    btn2 = [[
             InlineKeyboardButton("Vérifier les MP", url=f"telegram.me/{temp.U_NAME}")
           ]]
    reply_markup = InlineKeyboardMarkup(btn2)
    await query.message.edit_text(f"<b>Votre menu des paramètres pour {title} a été envoyé en MP</b>")  # ← Traduit
    await query.message.edit_reply_markup(reply_markup)
    if settings is not None:
        buttons = [
            [
    ...
    btn = [[
            InlineKeyboardButton("Indisponible", callback_data=f"unavailable#{from_user}"),  # ← Traduit
            InlineKeyboardButton("Uploadé", callback_data=f"uploaded#{from_user}")  # ← Traduit
         ],[
            InlineKeyboardButton("Déjà disponible", callback_data=f"already_available#{from_user}")  # ← Traduit
          ]]
    btn2 = [[
             InlineKeyboardButton("Voir le statut", url=f"{query.message.link}")  # ← Traduit
           ]]
    if query.from_user.id in ADMINS:
        user = await client.get_users(from_user)
        reply_markup = InlineKeyboardMarkup(btn)
        await query.message.edit_reply_markup(reply_markup)
        await query.answer("Voici les options disponibles !")  # ← Traduit
    else:
        await query.answer("Vous n'avez pas les droits suffisants pour faire ça !", show_alert=True)  # ← Traduit
    
elif query.data.startswith("unavailable"):
    ident, from_user = query.data.split("#")
    btn = [[
            InlineKeyboardButton("⚠️ Indisponible ⚠️", callback_data=f"unalert#{from_user}")  # ← Traduit
          ]]
    btn2 = [[
             InlineKeyboardButton('Rejoindre la chaîne', url=link.invite_link),  # ← Traduit
             InlineKeyboardButton("Voir le statut", url=f"{query.message.link}")  # ← Traduit
           ]]
    if query.from_user.id in ADMINS:
        user = await client.get_users(from_user)
        ...
        await query.message.edit_reply_markup(reply_markup)
        await query.answer("Marqué comme indisponible !")  # ← Traduit
        try:
            await client.send_message(chat_id=int(from_user), text=f"<b>Bonjour {user.mention}, votre demande est indisponible. Nos modérateurs ne peuvent pas l'uploader.</b>", reply_markup=InlineKeyboardMarkup(btn2))  # ← Traduit
        except UserIsBlocked:
            await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Bonjour {user.mention}, votre demande est indisponible. Nos modérateurs ne peuvent pas l'uploader.\n\nNote : Ce message est envoyé ici car vous avez bloqué le bot. Pour le recevoir en MP, débloquez le bot.</b>", reply_markup=InlineKeyboardMarkup(btn2))  # ← Traduit
    else:
        await query.answer("Vous n'avez pas les droits suffisants pour faire ça !", show_alert=True)  # ← Traduit
    ...
    ident, from_user = query.data.split("#")
    btn = [[
            InlineKeyboardButton("✅ Uploadé ✅", callback_data=f"upalert#{from_user}")  # ← Traduit
          ]]
    btn2 = [[
             InlineKeyboardButton('Rejoindre la chaîne', url=link.invite_link),  # ← Traduit
             InlineKeyboardButton("Voir le statut", url=f"{query.message.link}")  # ← Traduit
           ],[
             InlineKeyboardButton("Lien du groupe de demandes", url="https://t.me/+AntiFlix_d")  # ← Traduit
           ]]
    if query.from_user.id in ADMINS:
        user = await client.get_users(from_user)
        reply_markup = InlineKeyboardMarkup(btn)
        ...
        await query.message.edit_text(f"<b><strike>{content}</strike></b>")
        await query.message.edit_reply_markup(reply_markup)
        await query.answer("Marqué comme uploadé !")  # ← Traduit
        try:
            await client.send_message(chat_id=int(from_user), text=f"<b>Bonjour {user.mention}, votre demande a été uploadée par nos modérateurs. Cherchez dans notre groupe.</b>", reply_markup=InlineKeyboardMarkup(btn2))  # ← Traduit
        except UserIsBlocked:
            await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Bonjour {user.mention}, votre demande a été uploadée par nos modérateurs. Cherchez dans notre groupe.\n\nNote : Ce message est envoyé ici car vous avez bloqué le bot. Pour le recevoir en MP, débloquez le bot.</b>", reply_markup=InlineKeyboardMarkup(btn2))  # ← Traduit
    else:
        await query.answer("Vous n'avez pas les droits suffisants pour faire ça !", show_alert=True)  # ← Traduit
    ...
    ident, from_user = query.data.split("#")
    btn = [[
        InlineKeyboardButton("🟢 Déjà disponible 🟢", callback_data=f"alalert#{from_user}")  # ← Traduit
    ]]
    btn2 = [[
        InlineKeyboardButton('Rejoindre la chaîne', url=link.invite_link),  # ← Traduit
        InlineKeyboardButton("Voir le statut", url=f"{query.message.link}")  # ← Traduit
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.message.edit_text(f"<b><strike>{content}</strike></b>")
            await query.message.edit_reply_markup(reply_markup)
            await query.answer("Déjà disponible dans la base de données !")
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Bonjour {user.mention}, Votre demande est déjà disponible dans notre base de données. Veuillez effectuer une recherche dans notre groupe.</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Bonjour {user.mention}, Votre demande est déjà disponible dans notre base de données. Veuillez effectuer une recherche dans notre groupe.\n\nNote : Ce message est envoyé ici car vous avez bloqué le bot. Pour recevoir ces messages en privé, vous devez débloquer le bot.</b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await query.answer(f"Bonjour {user.first_name}, Votre demande a été uploadée !", show_alert=True)
        else:
            await query.answer("Vous n'avez pas les droits suffisants pour faire cela !", show_alert=True)
        ...
        try:
            ...
            button = [[
                ...
                InlineKeyboardButton("• Voir dans l'app web •", web_app=WebAppInfo(url=stream))
            ]]
            await query.message.edit_reply_markup(InlineKeyboardMarkup(button))
        except Exception as e:
            print(e)
            await query.answer(f"Une erreur s'est produite\n\n{e}", show_alert=True)
            return
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('VIP', callback_data="t.me/Kingcey"),
                InlineKeyboardButton('Groupe de films', url=GRP_LNK)
            ],[
                InlineKeyboardButton('Aide', callback_data='help'),
                InlineKeyboardButton('À propos', callback_data='about')
            ],[
                InlineKeyboardButton('Premium et Parrainage', callback_data='subscription')
            ],[
                InlineKeyboardButton('Rejoindre la chaîne', url=CHNL_LNK)
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('⤬ Ajoutez-moi à votre groupe ⤬', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ...
        if CLONE_MODE == True:
            buttons.append([InlineKeyboardButton('Créer un bot clone', callback_data='clone')])
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            InlineKeyboardButton('Filtres Manuel', callback_data='manuelfilter'),
            InlineKeyboardButton('Filtres Auto', callback_data='autofilter')
        ],[
        ...
        buttons = [[
             InlineKeyboardButton('⚙️ Admin uniquement 🔧', callback_data='admin'),
         ], [ 
             InlineKeyboardButton('Renommer', callback_data='r_txt'),   
             InlineKeyboardButton('Stream/Télécharger', callback_data='s_txt') 
         ], [ 
             InlineKeyboardButton('Stockage de fichiers', callback_data='store_file'),   
             InlineKeyboardButton('Telegraph', callback_data='tele') 
         ], [ 
             InlineKeyboardButton('Connexions', callback_data='coct'), 
             ...
             InlineKeyboardButton('ID Sticker', callback_data='sticker'),
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Kingcey")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    return await reply_msg.edit_text(f"**⚠️ Aucun fichier trouvé pour votre recherche - {name}**\n**Vérifiez l'orthographe svp.**")
        else:
            return
    else:
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton(f'𝑸𝒖𝒂𝒍𝒊𝒕é', callback_data=f"qualities#{key}"),
                InlineKeyboardButton("É𝒑𝒊𝒔𝒐𝒅𝒆𝒔", callback_data=f"episodes#{key}"),
                InlineKeyboardButton("𝑺𝒂𝒊𝒔𝒐𝒏𝒔", callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("𝑻𝒐𝒖𝒕 𝒆𝒏𝒗𝒐𝒚𝒆𝒓", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("𝑳𝒂𝒏𝒈𝒖𝒆𝒔", callback_data=f"languages#{key}"),
            InlineKeyboardButton("𝑨𝒏𝒏é𝒆𝒔", callback_data=f"years#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("𝑷𝑨𝑮𝑬", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝑺𝑼𝑰𝑽𝑨𝑵𝑻 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                )
            else:
                btn.append(
                    [InlineKeyboardButton("𝑷𝑨𝑮𝑬", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝑺𝑼𝑰𝑽𝑨𝑵𝑻 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
        ...
        btn.append(
            [InlineKeyboardButton(text="𝑷𝑳𝑼𝑺 𝑫𝑬 𝑷𝑨𝑮𝑬𝑺 𝑫𝑰𝑺𝑷𝑶𝑵𝑰𝑩𝑳𝑬𝑺",callback_data="pages")]
        )
